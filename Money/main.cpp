...
public:
    ...
    friend const ostream& operator<<(ostream& outputStream, const Money& m);
    friend const istream& operator>>(istream& inputStream, Money& m); //Your are modifying m from inputStream.
private:
...
const ostream& operator<<(ostream& outputStream, const Money& m) {
    int absDollars = abs(m.dollars);
    int absCents = abs(m.cents);
    outputStream<<"Your amount is: ";
...
const istream& operator>>(istream& inputStream, Money& m) {
    char dollarSign;
    ...
    cin >> amountAsDouble;
    m.dollars = m.dollarsPart(amountAsDouble);
    m.cents = m.centsPart(amountAsDouble);
    return inputStream;
}
